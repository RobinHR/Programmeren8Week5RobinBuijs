...
let fingerOne = document.getElementById('fingerOne')
let fingerTwo = document.getElementById('fingerTwo')

function fingerOneButtonHandler(e){
    e.preventDefault();
    console.log('Er is op de één vinger knop geklikt')
    machine.learn(predictionsArray, 'één vinger')
}

function fingerTwoButtonHandler(e){
    e.preventDefault();
    console.log('Er is op de twee vingers knop geklikt')
    machine.learn(predictionsArray, 'twee vingers')
}
...
function logData(predictions) {
    fingerOne.addEventListener('click', (event) => fingerOneButtonHandler(event, predictions));
    fingerTwo.addEventListener('click', (event) => fingerTwoButtonHandler(event, predictions));


    // code van Bas uit de les die ervoor zorgt dat de x en y as achter elkaar wordt geladen in een array
    ...
    // voorspellen code van Erik
    let prediction = machine.classify(predictionsArray)
    log.innerText = `Volgens mij is dit een ${prediction}`;
    ...
    // console.log(str);
    //
    // if (predictions[0].landmarks[4][1] > predictions[0].landmarks[20][1] && predictions[0].landmarks[16][1] && predictions[0].landmarks[12][1] && predictions[0].landmarks[8][1]) {
    //     log.innerText = "High five!"
    // } else {
    //     log.innerText = "Hier kan ik geen high five op geven!"
    // }
